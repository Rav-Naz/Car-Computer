  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children:
        widget.navigationItems.map((e) { return SizedBox(
            width: 50,
            height: 50,
            child: IconButton( iconSize: 35, color: e.icon == _currView ? Provider.of<UiProvider>(context).accentColor : Colors.white, icon: Icon(e.icon), onPressed: () {Provider.of<NavigationProvider>(context, listen: false).setView(e.view); _currView  = e.icon;}),
          );
        }).toList(),
      ),
      color: Colors.black,
