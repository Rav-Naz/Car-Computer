    ...
    return Container(
      ...
      child: Row(
        ...
        widget.navigationItems.map((e) { return SizedBox(
            width: 40,
            height: 40,
            child: IconButton( iconSize: 30, color: e.icon == _currView ? Provider.of<UiProvider>(context).accentColor : Colors.white, icon: Icon(e.icon), onPressed: () {Provider.of<NavigationProvider>(context, listen: false).setView(e.view); _currView  = e.icon;}),
          );
        }).toList(),
      ),
      color: Colors.black,
